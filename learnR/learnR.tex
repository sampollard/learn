...
The \verb|header = TRUE| indicates that the first line of this csv file contains the names of each column. If this is set to \verb|FALSE|, R sets the column names to \verb|V1|, \verb|V2|, and so on. The last argument is short for \emph{separator}. This may be whatever you want. Cryptically, a csv (comma separated value) file may not be separated by commas. Oftentimes, it is tab-separated. To accommodate this, one can say \verb|sep = "\t"|. The backslash here \vocab{escapes} the succeeding character \verb|t| and instead interprets this as a tab.
